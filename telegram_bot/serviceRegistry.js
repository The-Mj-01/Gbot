...
class ServiceRegistry{
    ...
    add(intent , ip , port){
        ...
        if(!this._services[key]){
            ...
            console.log(`Added service for intent ${intent} on ${ip}:${port} in ${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}`);
            this._cleanup();
            return;
        }

        this._services[key].timestamp= Math.floor(new Date()/1000);
        console.log(`Updated service for intent ${intent} on ${ip}:${port} in ${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}`);
        this._cleanup();
